...
function register(context, getPreferences, getFormatOptions, formatters) {
    return async (document, options) => {
        const sourceFile = sourceFile_1.createSourceFile(document, context);
    ...
    function getCssFormattingEdits(sourceFile, options) {
        const result = [];
        for (const sourceMap of sourceFile.getCssSourceMaps()) {
            ...
            for (const cssEdit of cssEdits) {
                const vueEdit = transforms_1.transformTextEdit(cssEdit, cssRange => sourceMap.getSourceRange(cssRange.start, cssRange.end));
                if (vueEdit) {
                    result.push(vueEdit);
                }
            }
    ...
    function getHtmlFormattingEdits(sourceFile, options) {
        const result = [];
        for (const sourceMap of sourceFile.getHtmlSourceMaps()) {
            const htmlEdits = formatters.html(sourceMap.mappedDocument, options);
            for (const htmlEdit of htmlEdits) {
                const vueEdit = transforms_1.transformTextEdit(htmlEdit, htmlRange => sourceMap.getSourceRange(htmlRange.start, htmlRange.end));
                if (vueEdit) {
                    result.push(vueEdit);
                }
            }
    ...
    function getPugFormattingEdits(sourceFile, options) {
        const result = [];
        for (const sourceMap of sourceFile.getPugSourceMaps()) {
            const pugEdits = formatters.pug(sourceMap.mappedDocument, options);
            for (const pugEdit of pugEdits) {
                const vueEdit = transforms_1.transformTextEdit(pugEdit, pugRange => sourceMap.getSourceRange(pugRange.start, pugRange.end));
                if (vueEdit) {
                    result.push(vueEdit);
                }
            }
