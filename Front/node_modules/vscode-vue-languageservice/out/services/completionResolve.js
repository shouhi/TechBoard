...
function register({ modules: { typescript: ts }, sourceFiles, getTsLs, vueHost }) {
    return async (item, newOffset) => {
        ...
        return item;
        async function getTsResult(sourceFile, vueItem, data) {
        ...
        async function getHtmlResult(sourceFile, vueItem, data) {
            var _a, _b;
        ...
        function getAutoImportResult(sourceFile, vueItem, data) {
            ...
            const componentName = shared_1.capitalize(shared_1.camelize(vueItem.label));
            ...
            if (descriptor.scriptSetup) {
                vueItem.additionalTextEdits = [
                    vscode.TextEdit.insert(textDoc.positionAt(descriptor.scriptSetup.loc.start + (scriptSetupImport ? scriptSetupImport.end : 0)), '\n' + insertText),
                ];
            }
            else if (descriptor.script) {
                vueItem.additionalTextEdits = [
                    vscode.TextEdit.insert(textDoc.positionAt(descriptor.script.loc.start + (scriptImport ? scriptImport.end : 0)), '\n' + insertText),
                ];
                const scriptRanges = scriptRanges_1.parseScriptRanges(ts, descriptor.script.content, descriptor.script.lang, true, true);
                ...
                if (exportDefault) {
                    ...
                    if (exportDefault.componentsOption && exportDefault.componentsOptionNode) {
                        ...
                        vueItem.additionalTextEdits.push(vscode.TextEdit.replace(vscode.Range.create(textDoc.positionAt(descriptor.script.loc.start + exportDefault.componentsOption.start), textDoc.positionAt(descriptor.script.loc.start + exportDefault.componentsOption.end)), unescape(printText.replace(/\\u/g, '%u'))));
                    }
                        ...
                        vueItem.additionalTextEdits.push(vscode.TextEdit.replace(vscode.Range.create(textDoc.positionAt(descriptor.script.loc.start + exportDefault.args.start), textDoc.positionAt(descriptor.script.loc.start + exportDefault.args.end)), unescape(printText.replace(/\\u/g, '%u'))));
                    }
                }
            }
            return vueItem;
            function planAInsertText() {
                ...
                const tsImportName = shared_1.camelize(path.basename(importFile).replace(/\./g, '-'));
    ...
    function getLastImportNode(code, lang) {
