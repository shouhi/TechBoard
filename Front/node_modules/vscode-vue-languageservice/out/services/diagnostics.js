...
function register({ sourceFiles, getCssLs, jsonLs, templateTsLs, scriptTsLs }, updateTemplateScripts) {
    ...
    return async (uri, response, isCancel) => {
        ...
        if (!worker) {
            worker = untrack_1.untrack(useDiagnostics(sourceFile));
            workers.set(sourceFile, worker);
        }
        worker(response, isCancel);
    };
    function useDiagnostics(sourceFile) {
        const { cssLsDocuments, cssLsSourceMaps, sfcJsons, sfcScriptForScriptLs, lastUpdated, sfcErrors, sfcTemplate, descriptor, document, sfcTemplateScript, templateScriptData, sfcScriptForTemplateLs, templateLsSourceMaps, } = sourceFile.refs;
        const templateTsProjectVersion = reactivity_1.ref();
        const scriptTsProjectVersion = reactivity_1.ref();
        ...
        const nonTs = [
            [useStylesValidation(reactivity_1.computed(() => cssLsDocuments.value)), 0, []],
            [useJsonsValidation(reactivity_1.computed(() => sfcJsons.textDocuments.value)), 0, []],
        ...
        let scriptTs = [
            ...
            [useScriptValidation(reactivity_1.computed(() => { var _a; return (_a = sfcScriptForScriptLs.textDocumentForSuggestion.value) !== null && _a !== void 0 ? _a : sfcScriptForScriptLs.textDocument.value; }), 3), 0, []],
            // [useScriptValidation(virtualScriptGen.textDocument, 4), 0, []], // TODO: support cancel because it's very slow
            [useScriptValidation(reactivity_1.computed(() => anyNoUnusedEnabled ? sfcScriptForScriptLs.textDocumentForSuggestion.value : undefined), 1, true), 0, []],
        ];
        ...
        function useJsonsValidation(documents) {
            const errors = reactivity_1.computed(async () => {
                var _a;
                let result = new Map();
            ...
            const cacheWithSourceMap = reactivity_1.computed(async () => {
                let result = [];
        ...
        function useScriptExistValidation() {
            const scriptErrors = reactivity_1.computed(() => {
                const diags = [];
            ...
            const templateErrors = reactivity_1.computed(() => {
                const diags = [];
