    ...
    async function onTs(uri, range, context) {
        ...
        for (const tsLoc of sourceFiles.toTsLocations(uri, range.start, range.end)) {
            ...
            if (tsLoc.type === 'embedded-ts' && !tsLoc.sourceMap.capabilities.organizeImports) {
                tsCodeActions = tsCodeActions.filter(codeAction => codeAction.kind !== vscode.CodeActionKind.SourceOrganizeImports
                    && codeAction.kind !== vscode.CodeActionKind.SourceFixAll);
            }
            const data = { lsType: tsLoc.lsType };
            for (const tsCodeAction of tsCodeActions) {
                if (tsCodeAction.title.indexOf('__VLS_') >= 0)
                    continue;
                const edit = tsCodeAction.edit ? rename_1.tsEditToVueEdit(tsLoc.lsType, tsCodeAction.edit, sourceFiles, () => true) : undefined;
